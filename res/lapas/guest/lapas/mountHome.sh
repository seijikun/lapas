...
if [ "$PAM_USER" != "$BASEUSER_NAME" ] && [ "$PAM_TYPE" == "open_session" ]; then
	...
	if [ ! -f "$USER_PERSISTENT_IMAGE" ]; then
		...
		assertSuccessfull /usr/sbin/mkfs.ext4 -m0 "$USER_PERSISTENT_IMAGE";
	fi
	...
	if [ $(mount | grep "$USER_BASE_MOUNT_DIR" | wc -l) == 0 ]; then
		...
		assertSuccessfull bindfs --map=${BASEUSER_UID}/${LOGINUSER_UID} -o ro,noatime --multithreaded "$USER_BASE_DIR" "$USER_BASE_MOUNT_DIR";
	fi
